...
function KindPanel({ kind }: { kind: CashPlanCategoryKind }) {
  ...
  React.useEffect(() => {
    void load()
  }, [kind, mockOrgId, mockUserId, mockUserRole])
  ...
  async function exportExcel() {
    try {
      ...
      downloadXlsxUint8(
        u8,
        kind === CashPlanCategoryKind.INCOME
          ? "资金计划流入类别.xlsx"
          : "资金计划流出类别.xlsx"
      )
      toast.success("已导出")
  ...
  async function downloadTemplate() {
    try {
      const prefix = kind === CashPlanCategoryKind.INCOME ? "IN" : "OUT"
      const u8 = await writeSimpleExcelBuffer([
        {
          name: kindLabel,
          headers: ["编码", "名称", "排序", "状态"],
          rows: [
            [`${prefix}_001`, `${kindLabel}示例A`, 10, "启用"],
            [`${prefix}_002`, `${kindLabel}示例B`, 20, "启用"],
            [`${prefix}_003`, `${kindLabel}示例C`, 30, "启用"],
            [`${prefix}_004`, `${kindLabel}示例D`, 40, "停用"],
            [`${prefix}_005`, `${kindLabel}示例E`, 50, "启用"],
          ],
        },
      ])
      downloadXlsxUint8(u8, `资金计划${kindLabel}类别_导入模板.xlsx`)
      toast.success("模板已下载")
    } catch {
      toast.error("模板下载失败")
    }
  ...
  async function onImportFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const row of first.rows) {
        ...
        if (old) {
          const res = await fetch(`/api/master-data/cash-plan-categories/${old.id}`, {
            method: "PUT",
            credentials: "include",
          ...
          const res = await fetch("/api/master-data/cash-plan-categories", {
            method: "POST",
            credentials: "include",
            headers: { ...baseHeaders, "Content-Type": "application/json" },
            body: JSON.stringify({ kind, code, name, sortOrder }),
          })
          const json = (await res.json()) as ApiSuccess<unknown> | ApiFail
          if (!json.success) throw new Error(json.error.message)
          createdCount += 1
  ...
  async function submitEdit() {
    ...
    try {
      const res = await fetch(
        `/api/master-data/cash-plan-categories/${editRow.id}`,
        {
          method: "PUT",
          credentials: "include",
          headers: { ...baseHeaders, "Content-Type": "application/json" },
  ...
  async function submitDelete() {
    ...
    try {
      const res = await fetch(
        `/api/master-data/cash-plan-categories/${deleteRow.id}`,
    ...
    <div className="space-y-4">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>新建{kindLabel}类别</DialogTitle>
            <DialogDescription>编码在该类型下唯一。</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>导入{kindLabel}类别</DialogTitle>
            <DialogDescription>请先下载模板并填写，再上传。</DialogDescription>
          </DialogHeader>
...
export function CashPlanCategoriesAdminClient() {
  return (
    <div className="space-y-4">
      <Alert>
        <AlertTitle>说明</AlertTitle>
        <AlertDescription className="text-sm">
          类别编码写入资金计划流入/流出明细的「类别」字段；编制页可从字典选择，亦支持历史自由文本。
        </AlertDescription>
      </Alert>
      <Tabs defaultValue="income">
        <TabsList>
          <TabsTrigger value="income">流入类别</TabsTrigger>
          <TabsTrigger value="expense">流出类别</TabsTrigger>
        </TabsList>
        <TabsContent value="income" className="mt-4">
          <KindPanel kind={CashPlanCategoryKind.INCOME} />
        </TabsContent>
        <TabsContent value="expense" className="mt-4">
          <KindPanel kind={CashPlanCategoryKind.EXPENSE} />
        </TabsContent>
      </Tabs>
    </div>
  )
