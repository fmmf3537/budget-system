import { approvalTransferBodySchema } from "@/lib/api/approval-schemas"
import { runTransfer } from "@/lib/api/approval-workflow"
...
export async function POST(request: Request, ctx: RouteCtx) {
  try {
    ...
    const parsed = approvalTransferBodySchema.safeParse(body)
    ...
    if (!result.ok) {
      ...
      if (result.code === "BAD_TARGET") {
        return fail("BAD_TARGET", "转交对象不存在或不属于当前组织", 400)
      }
      if (result.code === "NO_NODE") {
        return fail("NO_NODE", "当前待办缺少节点信息，无法转交", 400)
      }
